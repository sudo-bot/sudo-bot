 ...
 */
function sendFiles(octokit, commitMsg, files, defaultBranch = "master", targetBranch = "refs/heads/update/" + new Date().getTime()) {
    return new Promise((resolve, reject) => {
        const owner = process.env.OWNER;
        const repo = process.env.REPO;
        octokit.repos.getCommits({ owner, repo, sha: defaultBranch, per_page: 1 }).then(commitsres => {
            ...
            octokit.gitdata.createTree(
                {
                    owner, repo, tree: files.map(file => {
                        return {
                            mode: '100644',
                            type: 'blob',
                            path: file.path,
                    ...
                    let gpgMsg = 'tree ' + treeres.data.sha + "\n" +
                        'parent ' + lastCommit + "\n" +
                        'author ' + identity.name + ' <' + identity.email + '> ' + unixTime + ' +0000' + "\n" +
                        'committer ' + identity.name + ' <' + identity.email + '> ' + unixTime + ' +0000' + "\n\n" + commitMsg;
                    gpg.signCommit(gpgMsg).then(signature => {
                        octokit.gitdata.createCommit(
                            {
                                owner, repo, message: commitMsg,
                                tree: treeres.data.sha, parents: [lastCommit],
                            ...
                            octokit.gitdata.createReference(
                                { owner, repo, ref: targetBranch, sha: resultcommit.data.sha }
                            ).then(resultrefcreate => {
                                console.log("Ref-url: ", resultrefcreate.data.url);
                                resolve({
...
function createPullRequest(octokit, title, sourceBranch, targetBranch, message, mcm = true) {
    return new Promise((resolve, reject) => {
        ...
        octokit.pullRequests.create({
            ...
            body: message, maintainer_can_modify: mcm
        }).then(result => {
            resolve(result);
        }).catch(reject);
    });
